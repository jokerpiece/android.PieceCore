...
### ライブラリの作成
1. ![Sync Project](./mdImage/syncProj.png)ボタンを押下でビルドを実行する。
...
### アプリケーションに設定する方法
1. build_app.gradleをコピーしてbuild.gradleとして設定する。
2. AndroidManifest_app.xmlをコピーしてAndroidManifest.xmlとして設定する。
3. ビルドして実行できる。

### ライブラリに戻す方法
1. build_lib.gradleをコピーしてbuild.gradleとして設定する。
2. AndroidManifest_lib.xmlをコピーしてAndroidManifest.xmlとして設定する。
3. ビルドして「gradlew uploadArchives」を実行するとaarが作成できる。
