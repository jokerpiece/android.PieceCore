...
//        getActionBar().setTitle(R.string.goods_list);
	...
	private void showItemView() {
		...
		if(itemData.category_img_url != null){
			DownloadImageView categoryImage = new DownloadImageView(context);
			if(!categoryImage.setImageURL(itemData.category_img_url)){
	...
	private void getItemList(){
        ...
        getActivity().getLoaderManager().initLoader(Config.loaderCnt++, null, new LoaderCallbacks<ItemListData>(){
			...
			public Loader<ItemListData> onCreateLoader(int id, Bundle args) {
				 ...
				 if(couponID == null){
					 itemAPI =new ItemListAPI(context, categoryID, searchKeyword, nowPageCount);
				 }else{
					 itemAPI =new ItemListAPI(context, couponID, nowPageCount);
				 }
				 itemAPI.forceLoad();
				 return itemAPI;
				}
